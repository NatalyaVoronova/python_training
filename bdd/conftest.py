...
def stop(request):
    def fin():
        # Защищенная проверка перед вызовами методов
        if hasattr(fixture, 'session') and fixture.session:
            fixture.session.ensure_logout()
        if hasattr(fixture, 'destroy') and callable(fixture.destroy):
            fixture.destroy()
    # def fin():
    #     fixture.session.ensure_logout()
    #     fixture.destroy()
...
def pytest_addoption(parser):
    ...
    parser.addoption("--check_ui", action="store_true")  # если опция прописана в конфиг запуска - True, и False если отсутствует
